    ...
    class DynamoDB
      ...
      def key_schema(dynamo_db_table)
        ...
        if dynamo_db_table["key_schema"]
          ...
          attributes["hash_key"] = find_key(dynamo_db_table, "HASH") unless find_key(dynamo_db_table, "HASH").empty?
        end
        attributes
      end
      ...
      def sse_description(dynamo_db_table)
        ...
        if dynamo_db_table.sse_description
          if dynamo_db_table.sse_description.status == "ENABLED"
            ...
            attributes["server_side_encryption.0.enabled"] = true.to_s
          end
        else
          attributes["server_side_encryption.#"] = 0.to_s
        end
        attributes
      end
